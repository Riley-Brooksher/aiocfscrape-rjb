...
class CloudflareError(httpx.RequestError):
    pass
...
class CloudflareScraper(httpx.AsyncClient):
    ...
    async def request(self, method, url, *args, **kwargs):
        resp = await super(CloudflareScraper, self).request(method, url, *args, **kwargs)
    ...
    async def solve_cf_challenge(self, resp, **original_kwargs):
        ...
        if "Location" in redirect.headers:
            ...
            if 'cf_clearance' in redirect.headers['Set-Cookie']:
                resp = await self.request(self.org_method, submit_url, cookies = redirect.cookies)
                return resp
            else:
                return await self.request(method, submit_url, **original_kwargs)
        else:
            resp = await self.request(self.org_method, submit_url, **cloudflare_kwargs)
            return resp
    ...
    @classmethod
    async def get_cookie_string(cls, url, user_agent=None, **kwargs):
        """
        Convenience function for building a Cookie HTTP header value.
        """
        tokens, user_agent = await cls.get_tokens(url, user_agent=user_agent, **kwargs)
